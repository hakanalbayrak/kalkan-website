...
/**
 * One-time content SEO normalization. This migration is removed after it has
 * run successfully; the resulting taxonomy and metadata remain in WordPress.
 */
function kalkan_apply_content_seo_20260803() {
    if (get_option('kalkan_content_seo_20260803')) {
        return;
    }

    $announcements = get_category_by_slug('duyurular');
    if (! $announcements instanceof WP_Term) {
        return;
    }

    $posts = array(
        'spam-arama-engelleme' => array(
            'title'   => 'Spam Arama Engelleme – iPhone\'da Nasıl Yapılır?',
            'desc'    => 'iPhone’da spam aramaları engelleme, bilinmeyen arayanları tanıma, Kalkan kurulumu ve korumayı etkinleştirme adımlarını öğrenin.',
            'keyword' => 'spam arama engelleme',
        ),
        'numara-sorgulama' => array(
            'title'   => 'Numara Sorgulama – Bilinmeyen Numara Kimin?',
            'desc'    => 'Numara sorgulama yöntemlerini, bilinmeyen arayanlar hakkında hangi bilgilerin görülebileceğini ve şüpheli aramalarda nasıl davranacağınızı öğrenin.',
            'keyword' => 'numara sorgulama',
        ),
        'dolandirici-numara-tanima' => array(
            'title'   => 'Dolandırıcı Numara Tanıma – Korunma Rehberi',
            'desc'    => 'Telefon dolandırıcılığı işaretlerini tanıyın; şüpheli aramalarda kişisel bilgilerinizi ve hesaplarınızı korumak için uygulayabileceğiniz adımları öğrenin.',
            'keyword' => 'dolandırıcı numara tanıma',
        ),
        'bilinmeyen-numara-kimin' => array(
            'title'   => 'Bilinmeyen Numara Kimin? – Araştırma Rehberi',
            'desc'    => 'Bilinmeyen bir numaranın kime ait olabileceğini araştırma yöntemlerini, kurumsal arayanları tanıma yollarını ve dikkat edilmesi gereken işaretleri öğrenin.',
            'keyword' => 'bilinmeyen numara kimin',
        ),
        'surekli-arayan-numara-engelleme' => array(
            'title'   => 'Sürekli Arayan Numara Engelleme – iPhone Rehberi',
            'desc'    => 'iPhone’da sürekli arayan ve rahatsız eden numaraları engelleme, sessize alma ve istenmeyen çağrıları azaltma yöntemlerini adım adım öğrenin.',
            'keyword' => 'sürekli arayan numara engelleme',
        ),
        'kalkan-uygulamasi-yayinda' => array(
            'title'   => 'Kalkan Uygulaması Yayında – iOS Arama Koruması',
            'desc'    => 'Kalkan’ın App Store sürümünü, ücretsiz Genel Koruma ile Premium Ekstra Koruma farkını ve iPhone’da istenmeyen aramaları azaltma özelliklerini keşfedin.',
            'keyword' => 'Kalkan uygulaması',
        ),
        'kalkan-sizi-nasil-korur' => array(
            'title'   => 'Kalkan Sizi Nasıl Korur? – Arama Koruma Rehberi',
            'desc'    => 'Kalkan’ın istenmeyen aramaları azaltmaya ve kurumsal arayanları tanımaya nasıl yardımcı olduğunu, kurulumu ve sunduğu günlük faydaları öğrenin.',
            'keyword' => 'Kalkan nasıl korur',
        ),
        'bilinmeyen-bir-numara-aradiginda-ne-yapmalisiniz-7-basit-adim' => array(
            'title'   => 'Bilinmeyen Numara Aradığında Ne Yapmalı? 7 Güvenli Adım',
            'desc'    => 'Bilinmeyen bir numara aradığında kişisel bilgilerinizi korumak, arayanı değerlendirmek ve şüpheli çağrılarda güvenli hareket etmek için 7 adımı öğrenin.',
            'keyword' => 'bilinmeyen numara aradığında ne yapmalı',
        ),
    );

    foreach ($posts as $slug => $seo) {
        $post = get_page_by_path($slug, OBJECT, 'post');
        if (! $post instanceof WP_Post) {
            continue;
        }

        wp_set_post_categories($post->ID, array((int) $announcements->term_id), true);
        wp_update_post(array('ID' => $post->ID, 'post_excerpt' => $seo['desc']));
        update_post_meta($post->ID, '_seopress_titles_title', $seo['title']);
        update_post_meta($post->ID, '_seopress_titles_desc', $seo['desc']);
        update_post_meta($post->ID, '_seopress_analysis_target_kw', $seo['keyword']);
        delete_post_meta($post->ID, '_thumbnail_id');
        delete_post_meta($post->ID, '_seopress_social_fb_img');
        delete_post_meta($post->ID, '_seopress_social_twitter_img');
    }

    update_term_meta($announcements->term_id, '_seopress_titles_title', 'Duyurular – Telefon Güvenliği ve Kalkan Rehberleri');
    update_term_meta($announcements->term_id, '_seopress_titles_desc', 'İstenmeyen aramalar, bilinmeyen numaralar, telefon dolandırıcılığı ve Kalkan kullanımı hakkında güncel rehberleri okuyun.');
    update_option('kalkan_content_seo_20260803', true, false);
}
add_action('init', 'kalkan_apply_content_seo_20260803', 40);
